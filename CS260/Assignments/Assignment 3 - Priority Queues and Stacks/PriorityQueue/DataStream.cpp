...
Packet DataStream::getPacket()
{
  ...
  p.data = randomHexString(gen, 16) + "-" + randomHexString(gen, 16) + "-"
           + randomHexString(gen, 16) + "-" + randomHexString(gen, 16);
  return p;
}

DataStream::DataStream(unsigned int sourceSeed): gen(sourceSeed)
{
}
